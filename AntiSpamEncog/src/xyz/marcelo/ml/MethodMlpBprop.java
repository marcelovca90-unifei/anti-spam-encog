...
public class MethodMlpBprop
{
    ...
    public static void run(String folder, BasicMLDataSet trainingSet, BasicMLDataSet validationSet, BasicMLDataSet testSet, int seed)
    {
        ...
        for (int h1 = Math.min(inputCount, outputCount); h1 <= Math.max(inputCount, outputCount); h1++)
        {
            for (int h2 = 1; h2 <= h1; h2++)
            {
                ...
                logger.info(String.format("%s @ %d\t%s\tHP: %.2f%% (%d/%d)\tSP: %.2f%% (%d/%d)\tGP: %.2f%%", structure, seed,
                        folder.replace(Folders.BASE_FOLDER, ""), hamPrecision, hamCorrect, hamCount, spamPrecision, spamCorrect, spamCount, precision));
        ...
        logger.info(String.format("%s @ %d\t%s\tHP: %.2f%%\tSP: %.2f%%\tGP: %.2f%%", bestStructure, seed, folder.replace(Folders.BASE_FOLDER, ""),
                bestHamPrecision, bestSpamPrecision, bestPrecision));
    }
}
